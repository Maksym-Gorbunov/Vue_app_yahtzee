  ...
  <div id="app">
    ...
    <div id="right" >
      ...
      <div id="info">
        <h5>
          Maksym Gorbunov
          <a href="http://www.yahtzee.org.uk/rules.html" target="_blank" id="info_icon" ><i class="fas fa-info-circle"></i></a>
        </h5>
      </div> 
    </div>
...
export default {
  ...
  data(){
    return {
      ...
      // bonus_for_second_yahtzee_required: 63,
  ...
  methods: {
    ...
    rollOnTableWithKey(){
      console.log('roll______table')  
      console.log(this.selected_item.id)  
      console.log(this.selected_item.combination)  
      console.log(this.selected_item.possible)  
          ...
          case space: 
            if(that.space_btn && !that.pressed){
              if(that.cursor_on_table == true){
                ...
                setTimeout(() => { that.rollBtn.classList.remove('btn_active')} ,150 )
                that.rollBtn.click() 
                break;
              }
            }
    ...
    yahtzeeBonus(item){
      if(this.bonus_for_second_yahtzee_allowed && item.poings == 50){
        ...
        this.items[this.items.length-1].poings += this.bonus_for_second_yahtzee
